...
Optional R acceleration is provided by the companion **ShennongOpt** package
and is never activated during package loading. When an accelerated workflow
first reaches a covered hot path—Seurat RunPCA, ScaleData,
FindVariableFeatures, FindNeighbors, FindClusters, scran, decontX,
scDblFinder, Coralysis, UCell, LISI, or Rogue—Shennong checks the installed
ShennongOpt build and the upstream package version, and activates only
validated, operation-scoped patches for that single workflow call; the pre-call
patch state is restored afterward. SeuratObject merge/JoinLayers patches are
available through explicit `sn_enable_acceleration()` requests. Uncovered hot
paths always run plain upstream, so acceleration never changes scientific
contracts.
...
The opt-outs do not deactivate a patch that you activated manually; explicit
`sn_enable_acceleration()` and `sn_with_acceleration()` calls are intentional
overrides that ignore them. Thread budgets can be set through the
`shennong.opt.threads` option or the `SHENNONG_OPT_THREADS` environment
variable. Shennong does not install ShennongOpt, its upstream packages, or any
Python environment.
...
The recorder instruments every export except its ten usage-control APIs (265
of the current 275 exports): analysis,
...
remote security, nesting, privacy and acceleration evidence semantics.
