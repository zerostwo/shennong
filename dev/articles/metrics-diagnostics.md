...
metrics. This article uses the public Kotliarov PBMC CITE-seq subset.
`real_batch` is a source acquisition label, `real_sample` identifies the
20 biological samples, and RNA-versus-WNN clusters provide a real
cross-modality comparison without inventing reference labels.
...
sections: `summary`, `per_cell`, `per_group`, and `parameters`. Here the
dependency-free panel evaluates the unintegrated PCA space; the same
call can be pointed at an integrated reduction later.
...
a label. The upstream `lisi` package is optional, so this real-data
chunk runs only in the `all` profile when that package is installed.
...
computed clusterings. Here RNA-only clusters are compared with WNN
clusters learned from the aligned RNA and ADT assays; neither is
mislabeled as ground truth.
...
When a curated reference label becomes available, repeat the same calls
with that label and report its provenance. The cross-modality comparison
above is a stability diagnostic, not an accuracy claim.
