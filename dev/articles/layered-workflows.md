...
This article uses real RNA and ADT count layers from the public
Kotliarov PBMC CITE-seq fixture. It does not copy counts into a pretend
corrected layer. A deterministic, sample-stratified subset keeps the
layer examples fast while preserving cells from all 20 biological
samples.
...
The object retains measured RNA and ADT counts. Normalized `data` layers
are created only by the workflows that require them.
...
CellTypist and ROGUE are optional backends. They run only in the `all`
profile when the executable/model or R dependency is already available.
