...
source-study `real_response` value. Cell counts are never treated as
replicate counts for the response comparison.
...
The design table above is the source-study metadata used below. Its
one-row-per- sample structure makes the inferential unit explicit.
...
The plotting helper can also keep the biological sample as the
replicate. The following computes per-sample cluster proportions from
metadata, fills absent sample/cluster combinations with zero, and then
draws the sample distributions within each response group.
...
`flow_by = c("level1", "level2", "level3")` to audit hierarchical
annotation or source-to-final-label flows; no optional Sankey backend is
needed.
...
abundance. Because miloR is optional, these chunks run only in the `all`
profile when the backend is installed.

``` r

milo_result <- sn_run_milo(
  x = pbmc,
  sample_by = "real_sample",
  group_by = "real_response",
  contrast = c("d0 high", "d0 low"),
...
head(sn_get_milo_result(milo_result))
```

`return_object = FALSE` returns a complete result; use
[`sn_get_milo_result()`](https://zerostwo.github.io/shennong/dev/reference/sn_get_milo_result.md)
for its table. Add `keep_model = TRUE` to retain the fitted Milo object
in `milo_result$models$milo`. The seed controls sampling and
...
pbmc <- sn_store_result(
  object = pbmc, type = "milo",
  result = milo_result, result_id = "response_da"
)
sn_list_results(pbmc, type = "milo")
...
in variables work too; no `aes()` or unquoted column expressions are
needed.
...
All composition plot text defaults to **8 pt**, including axes, titles,
legends, facet strips and Sankey node labels. C-style source headings
use the same size. For backward compatibility `stratum_label_size` still
takes millimeters; its new default is `8 / ggplot2::.pt` to produce 8 pt
text.

C-style bottom pies are optional through an explicit
...
## Retrieve one Milo annotation

The column supplied to `sn_store_milo(annotation_by = "cell_type")` also
controls annotation filtering during retrieval. Annotation and spatial
FDR filters can be used independently. Missing stored annotation columns
cause an explicit error when annotation filtering is requested.

``` r

sn_list_results(pbmc, type = "milo")
sn_get_milo_result(pbmc, "response_da", annotation = "Tcell")
```
