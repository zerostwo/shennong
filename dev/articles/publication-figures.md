...
The example above is rendered from 2,000 real CITE-seq cells and retains
the study’s responder and acquisition-batch labels. Point size follows a
clamped power rule, and high-density layers are recommended for
rasterization while text and geometry remain vector in PDF/SVG output.
...
sn_get_figure_spec(plot)$recommended
...
RStudio graphics device. PDF uses base R’s portable device. SVG uses the
optional `svglite` package rather than a platform-specific X11/Cairo
device; when it is unavailable, Shennong stops before drawing with an
actionable dependency message. Explicit dimensions override automatic
values.
