...
- result_id:
  ...
  Stable identifier for the stored deconvolution result when `x` is a
...
- max_dense_gb:
  ...
  Maximum estimated size, in GiB, of any expression matrix that may be
  materialized as a dense double matrix. Large sparse inputs fail before
  allocation unless the caller explicitly raises this budget.
