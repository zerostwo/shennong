...
vignette: >
  %\VignetteIndexEntry{Choose a backend}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

Start with [Get started](get-started.html) for a complete small example.
Use this catalog when choosing software for a specific analysis. A listed
adapter is not a claim that the backend is installed or validated for every
input; check its requirements and recorded evidence before running it.
...
For shared argument names and backend control lists, see
[Parameters and results](parameters-and-results.html). For result schemas,
validation, and migration, see [Manage stored results](analysis-results-and-methods.html).
