...
- `sn_find_doublets()`: doublet detection via `scDblFinder` (default) or Scrublet through scanpy's native wrapper (`method = "scrublet"`, managed `scrublet` pixi environment, raw counts only, outputs `scrublet.class`/`scrublet.score`); native automatic
  ...
  materialized per sample, with `n_workers = 1` providing the lowest peak memory
...
- `sn_run_scvi()` / `sn_run_scanvi()` / `sn_run_scpoli()`: explicit wrappers for the corresponding `sn_run_cluster()` integration methods.
...
- `sn_compare_integrations()`: run a shared-cell, shared-feature scib-metrics
  benchmark over the native reductions retained by multi-method/parameter-grid
  clustering. It evaluates each unique embedding once within its preprocessing
  group and matching unintegrated baseline, then maps scores to all resolution
  runs sharing that embedding. The output records run, embedding,
  preprocessing, baseline, within-group rank, and overall rank. Retrieve it
  with `sn_list_results(type =
...
- `sn_run_enrichment()`: ORA or GSEA from vectors, tables, or stored DE; stored-
  ...
  numeric ranking formula selects GSEA automatically; `mapping = gene ~ score |
  group` ranks each group independently. Use explicit `analysis = "ora"` for
  numeric group codes. Generated IDs combine source, mode, and database.
...
- Clustering and scVI/scANVI/scPoli shortcuts use `batch_by`, `backend_control`,
  and top-level `seed`; the shortcuts all run the clustering workflow.
- Common worker controls use `n_workers`; backend-native thread settings stay
  inside their backend control lists.
- `sn_get_result(object, type, result_id = NULL)` retrieves complete results;
  omitted IDs require exactly one candidate. Typed result getters return
  tables and have no `with_metadata` switch.
- DE/enrichment table selection uses `top_scope`, `p_adjusted_cutoff`, and
  `top_n`; DE additionally supports independent `direction`/`logfc_threshold`.
- `sn_score_programs(group_by = ..., aggregate = "expression" | "scores")`
  makes the order of aggregation explicit. The scoring seed is top-level.
- `sn_store_result(overwrite = TRUE)` deliberately replaces an existing key.
  Default scoring/Milo runs receive unique IDs; scoring replacement also
  reconciles owned metadata columns.
