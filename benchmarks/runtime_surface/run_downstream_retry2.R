...
if (!is.null(big)) {
  ids <- head(as.character(sort(unique(as.numeric(as.character(big$seurat_clusters))))), 4L)
  signatures <- list()
  stored_analysis <- NULL
  for (id in ids[-1]) {
    store <- paste0("markers_c", id)
    res <- log_step(paste0("de:wilcox:c", id, "-vs-c", ids[[1]]),
  ...
  if (length(signatures)) {
    ...
    log_step("enrich:ora-gobp",
             sn_enrich(big, analysis = stored_analysis, database = "GOBP"))
    log_step("enrich:gsea-hallmark",
             sn_enrich(big, analysis = stored_analysis, database = "Hallmark"))
  }
}
