...
- Optional rasterization must not make feature plotting unavailable. Shennong
  builds the vector layer first and applies `ggrastr` only when installed, so
  `raster = TRUE` retains a valid fallback in minimal CI and user environments.
