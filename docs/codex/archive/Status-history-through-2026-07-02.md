# Shennong Modernization Status (history through 2026-07-02)

Last updated: 2026-07-02

## 2026-07-02

- Fixed SCTransform clustering feature blocking: `sn_run_cluster(normalization_method = "sctransform")` now resolves and applies `block_genes` before recording the final SCT HVG/PCA feature set, while preserving explicit `hvg_features` overrides.
- Added a focused clustering regression test, updated roxygen/man docs, the clustering vignette, shipped package-skill notes, and NEWS for the corrected SCT `block_genes` behavior.
- Validation: focused clustering tests pass with `FAIL 0 | WARN 0 | SKIP 0 | PASS 432`; full local `testthat::test_local()` passes with `FAIL 0 | WARN 0 | SKIP 2 | PASS 1432`; roxygen documentation regenerated successfully with `devtools::document()`; pkgdown site rebuild completed with `override = list(template = list(package = "pkgdown"))` after the default tidytemplate path hit the known Google Fonts TLS failure.
